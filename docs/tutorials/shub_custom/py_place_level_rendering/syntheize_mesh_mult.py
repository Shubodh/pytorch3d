...
# def redner_py3d_img(img_size, param, dest_file, mesh_dir, device):
def redner_py3d_img(img_size, param, dest_file, mesh_dir, device = None):
	...
	lights = lights_given_position(RT[0:3, 3])
	if device is not None:
...
def viz_image(RT_list, camera, dest_dir, mesh_dir):
	"""
	...
	for i, RT in enumerate(RT_list):
		...
		redner_py3d_img(img_size, param, RT, dest_file, mesh_dir)


def viz_points_cam(centroids_coordinates, sphere_center_coords, mesh, camera, dest_dir, mesh_dir):
	...
	viz_image(list_of_rts, camera, dest_dir, mesh_dir)
...
def synth_image(viz_pcd=False, custom_dir=False):
	...
	viz_points_cam(centroids_coordinates, sphere_center_coords, mesh, camera, dest_dir, mesh_dir)
